...
MainActiveNode::MainActiveNode() : Node("Main_Active")
{
  ...
  command_mode = RPM_COMMAND; // TEST, TORQUE, POSITION, RPM
  ...
  for (int i = 0; i < NUM_OF_CANABLE; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      ...
      std::string speed_topic = "/can" + std::to_string(i + 4) + "/motor" + std::to_string(j + 1) + "_speed"; // rpm>>speed로 가자미
...
{
  if(drive_mode == STOP||drive_mode == FLIPPER_MODE) 
...
{
  if(msg->buttons[0]==1) drive_mode = FLIPPER_MODE;
  else if(msg->buttons[1]==1) drive_mode = STAIR_MODE;
  else if(msg->buttons[2]==1) drive_mode = STOP;
  else if(msg->buttons[3]==1) drive_mode = NORMAL_MODE;

  // Toggle flipper mode
  

  // Select flipper for adjustment
  if (drive_mode == FLIPPER_MODE)
  {
    for (int i = 4; i < 7; i++)
    {
      if (joystick_data_->buttons[i])
      {
        selected_flipper = i-4;
        break;
      }
    }
...
void *MainActiveNode::pthread_main()
{
  ...
  while ((!main_thread_exit))
  {
    ...
    // Read buffer //여기가 바껴야함 332  여서 바껴야함 
    for (int i = 0; i < NUM_OF_CANABLE; i++)
    {
      for (int j = 0; j < NUM_OF_MOTOR; j++)
      {
    ...
    for (int i = 0; i < NUM_OF_CANABLE; i++) // 그럴필요없이 canable이 2일때 증가 3까지 안가게 코딩
    {
      for (int j = 0; j < NUM_OF_MOTOR; j++)//why? ++j?
      {
         if(i == 2 && j == 2) break; // 강제 종료 구문  
        
        if (i == 0)
        {
          if (j < 2)
          {
            Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[i], j + 1, left_rpm_command_, frame[i]); //3,1 왼쪽
          }
          else
          {
            Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[i], j + 1, right_rpm_command_, frame[i]); // 3,2 3,3 오른쪽
          }
        }
        else if(i == 1 && j == 1)
        {
           Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[i], j + 1, left_rpm_command_, frame[i]); // 4,2 왼쪽 보내기 
        }
        else
        {
          if (joystick_data_ && joystick_data_->buttons[4]) // 여기부터는 토크 계산부분 잘 모름....
          {
            float torque_adjustment = joystick_data_->axes[1];
