...
MainActiveNode::MainActiveNode() : Node("Main_Active")
{
  ...
  int can4_core = 1;      // Set the desired CPU core for can4
  int can4_priority = 99; // Set the desired thread priority

  int can5_core = 2;      // Set the desired CPU core for can5
  int can5_priority = 98; // Set the desired thread priority

  int can6_core = 3;      // Set the desired CPU core for can6
  int can6_priority = 97; // Set the desired thread priority
  ...
  usleep(500);
  // start Main thread
  can4_thread = Thread_init([](void *arg) -> void *
                            { return static_cast<MainActiveNode *>(arg)->pthread_can4(); },
                            this, can4_core, policy, can4_priority);
  can5_thread = Thread_init([](void *arg) -> void *
                            { return static_cast<MainActiveNode *>(arg)->pthread_can5(); },
                            this, can5_core, policy, can5_priority);
  can6_thread = Thread_init([](void *arg) -> void *
                            { return static_cast<MainActiveNode *>(arg)->pthread_can6(); },
                            this, can6_core, policy, can6_priority);
  // can5_thread = Thread_init(&MainActiveNode::can5_thread_func, this, can5_core, policy, can5_priority);
...
MainActiveNode::~MainActiveNode()
{
  ...
  can4_thread_exit = true;
  can5_thread_exit = true;
  can6_thread_exit = true;

  // Waiting off the threads and join
  if (pthread_join(can4_thread, NULL) != 0)
  {
    std::cerr << "Failed to join CAN4 thread" << std::endl;
  }
  if (pthread_join(can5_thread, NULL) != 0)
  {
    std::cerr << "Failed to join CAN5 thread" << std::endl;
  }
  if (pthread_join(can6_thread, NULL) != 0)
  {
    std::cerr << "Failed to join CAN6 thread" << std::endl;
  }
...
void *MainActiveNode::pthread_can4()
{
    ...
    { // Set all torques and speeds to zero if no joystick input for 0.3 seconds
      for (int i = 0; i < NUM_OF_CANABLE; i++)
      {
        ...
        for (int j = 1; j < NUM_OF_MOTOR; j++)
        {
            ...
            if (motorID == 2)
            {
                Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[0], motorID, left_rpm_command_, frame[0]);
            }
            else if (motorID == 3)
            {
                Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[0], motorID, right_rpm_command_, frame[0]);
            }
        }

        // Handle flipper torque
        int canable, motor;
        get_flipper_info(0, canable, motor);
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[canable], motor, torque_command_[0], frame[canable]);
...
void *MainActiveNode::pthread_can5()
{
    struct period_info pinfo;
    pinfo.period_ns = ns_TenMilSec; // 10ms
    usleep(500);
    ...
    while ((!can5_thread_exit))
    {
        ...
        for (int j = 1; j < NUM_OF_MOTOR; j++)
        {
            ...
            if (motorID == 2)
            {
                Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[1], motorID, left_rpm_command_, frame[1]);
            }
            else if (motorID == 3)
            {
                Rmd.RPM_CLOSED_LOOP_CONTROL(cansock[1], motorID, right_rpm_command_, frame[1]);
            }
        }

        // Handle flipper torque
        int canable, motor;
        get_flipper_info(2, canable, motor);
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[canable], motor, torque_command_[2], frame[canable]);
...
void *MainActiveNode::pthread_can6()
{
    struct period_info pinfo;
    pinfo.period_ns = ns_TenMilSec; // 10ms
    usleep(500);
