...
MainActiveNode::MainActiveNode() : Node("Main_Active")
{
  ...
  // set can
  cansock[0] = CAN_INIT("can4"); // front flippers
  cansock[1] = CAN_INIT("can5"); // front wheels
  cansock[2] = CAN_INIT("can6"); // back wheels
  cansock[3] = CAN_INIT("can7"); // back flippers
...
/*void MainActiveNode::joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
{
    ...
    // 추가된 코드 - JoyToControlNode 클래스의 기능 통합
    float wheel_base = 0.5;
    float wheel_radius = 0.1;

    float linear_velocity = msg->axes[1] * 1000.0;
    float angular_velocity = msg->axes[2] * 1000.0;

    float left_wheel_rpm = ((linear_velocity - (wheel_base / 2.0) * angular_velocity) / (2.0 * M_PI * wheel_radius)) * 1.0;
    float right_wheel_rpm = ((linear_velocity + (wheel_base / 2.0) * angular_velocity) / (2.0 * M_PI * wheel_radius)) * 1.0;

    auto rpm_msg = std_msgs::msg::Float32MultiArray();
    rpm_msg.data = {static_cast<float>(static_cast<int32_t>(left_wheel_rpm)), static_cast<float>(static_cast<int32_t>(right_wheel_rpm))};
    rpm_publisher_->publish(rpm_msg);

    for (int i = 0; i < 4; ++i)
    {
        button_states_[i] = msg->buttons[20 + i] != 0;
    }
    
    // Store the state of button[9], button[10], button[2], and button[12]
    button_9_state_ = msg->buttons[9] != 0;
    button_10_state_ = msg->buttons[10] != 0;
    if (msg->buttons[2] != 0)
    {
    ...
    if (msg->buttons[12] != 0)
    {
        ...
        flipper_torques_.fill(0.0f);
        RCLCPP_INFO(this->get_logger(), "\033[1;32mReady to go.\033[0m");
    }
...
MainActiveNode::~MainActiveNode()
{
  ...
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      ...
      std::cout << "[" << i << "-" << j << "]"
                << "Motor Stop... " << std::endl;
      Rmd.WRITE_MOTOR_OFF(cansock[i], j, frame[i]);
  ...
  // close socket can
  close(cansock[0]); // can4
  close(cansock[1]); // can5
  close(cansock[2]); // can6
  close(cansock[3]); // can7
...
void *MainActiveNode::pthread_main()
{
  ...
  while ((!main_thread_exit))
  {
    ...
    for (int i = 1; i < 3; i++) // CAN IDs 5, 6 correspond to index 1~2 4 5 6 7
    {
   ...
   /* if (flipper_pos == false)
    {
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[0], 1, torque_command_[0], frame[0]); // i = 0
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[0], 2, torque_command_[1], frame[0]); // i = 1
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[3], 1, torque_command_[2], frame[3]); // i = 2
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[3], 2, torque_command_[3], frame[3]); // i = 3
    }
    ...
    else
    {
      ...
      else
      {
        ...
        // joint[0].RefPos = joint[0].RefPos;
        // joint[0].RefVel = joint[0].RefVel;
        // joint[1].RefPos = joint[1].RefPos;
        // joint[1].RefVel = joint[1].RefVel;
      }
      ...
      // for (int i = 0; i < 4; i++)
      // {
      //   Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[i], 2, tau[i][j], frame[i]);
      // }
    //}
...
void *MainActiveNode::pthread_arm()
{
  ...
  while ((!arm_thread_exit))
  {
    ...
    q0(1) = motor_state[5].theta[0];
    q0(2) = motor_state[5].theta[1];
    q0(3) = motor_state[6].theta[0];
    q0(4) = motor_state[6].theta[1];
    q0(5) = motor_state[7].theta[0]; // 이걸 joint 구조체에 넣고 joint를 쓰면 좋을 것 같다. 수정하자 --> 안됨
    ...
    joint[0].CurrentPos = motor_state[4].theta[0];
    joint[0].CurrentVel = motor_state[4].speed[0];
    joint[1].CurrentPos = motor_state[5].theta[0];
    joint[1].CurrentVel = motor_state[5].speed[0];
    joint[2].CurrentPos = motor_state[5].theta[1];
    joint[2].CurrentVel = motor_state[5].speed[1];
    joint[3].CurrentPos = motor_state[6].theta[0];
    joint[3].CurrentVel = motor_state[6].speed[0];
    joint[4].CurrentPos = motor_state[6].theta[1];
    joint[4].CurrentVel = motor_state[6].speed[1];
    joint[5].CurrentPos = motor_state[7].theta[0];
    joint[5].CurrentVel = motor_state[7].speed[0];
    ...
    if (runtime == 0.0)
    {
      ...
      joint[0].InitPos = motor_state[4].theta[0];
      joint[0].InitVel = motor_state[4].speed[0];
      joint[1].InitPos = motor_state[5].theta[0];
      joint[1].InitVel = motor_state[5].speed[0];
      joint[2].InitPos = motor_state[5].theta[1];
      joint[2].InitVel = motor_state[5].speed[1];
      joint[3].InitPos = motor_state[6].theta[0];
      joint[3].InitVel = motor_state[6].speed[0];
      joint[4].InitPos = motor_state[6].theta[1];
      joint[4].InitVel = motor_state[6].speed[1];
      joint[5].InitPos = motor_state[6].theta[0];
      joint[5].InitVel = motor_state[6].speed[0];
    }
