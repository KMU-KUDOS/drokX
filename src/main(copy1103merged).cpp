...
MainActiveNode::MainActiveNode() : Node("Main_Active")
{
  ...
  for (int i = 0; i < JOINT_NUM; i++) // JOINT_NUM = 6
  {
  ...
  // Read encoder value
  // 차체 6개(뒷 플리퍼 ㅂㅂ) 팔 7개(baseㅂㅂ) 캐너블 7개
  // 차체 6개(2 2 2), 팔 6개(1 2 / 2 1), 캐너블 7개(3 4) NUM_OF_CANABLE = 7
  std::cout << "========== Current Q ==========" << std::endl;
  for (int i = 0; i < 5; i++) // 0 1 2 // 3(1) 4(2) 5(2) 6(1)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
  ...
  for (int i = 5; i < NUM_OF_CANABLE; i++) // NUM_OF_CANABLE = 7
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      Rmd.REQUEST_MOTOR_STATUS(cansock[i], j + 1, frame[i]);
...
void MainActiveNode::cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  if (!cmd_vel_flag)
  {
    std::cout << "cmd_vel_callback is disabled until 9000ms runtime" << std::endl;
    return; // cmd_vel_flag가 false면 동작 중단
  }

  double wheel_base = 0.5;   // 로봇의 휠 간 거리
...
void *MainActiveNode::pthread_main()
{
  ...
  while (!main_thread_exit)
  {
    ...
    // Control logic for wheels (CAN IDs 4 and 5)
    for (int i = 1; i < 3; i++)
    {
    ...
    // Handle flipper torque control (CAN IDs 4 and 7)
    for (int i = 0; i < 4; i++)
    {
      if (i == 0) // 앞쪽 왼쪽 플리퍼 (can4, motor 1)
      {
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[0], 1, torque_command_[0], frame[0]);
      }
      else if (i == 1) // 앞쪽 오른쪽 플리퍼 (can4, motor 2)
      {
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[0], 2, torque_command_[1], frame[0]);
      }
      else if (i == 2) // 뒤쪽 왼쪽 플리퍼 (can7, motor 1)
      {
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[3], 1, torque_command_[2], frame[3]);
      }
      else if (i == 3) // 뒤쪽 오른쪽 플리퍼 (can7, motor 2)
      {
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[3], 2, torque_command_[3], frame[3]);
      }
    }
...
void *MainActiveNode::pthread_arm()
{
  ...
  while ((!arm_thread_exit))
  {
    ...
    for (int i = NUM_OF_CANABLE - 3; i < NUM_OF_CANABLE; i++) // 3~6
    {
      for (int j = 0; j < NUM_OF_MOTOR; j++)
      {
    ...
    // Lock the mutex only when accessing shared resources
    {
      ...
      joint[0].CurrentPos = motor_state[3].theta[1]; // elbow
      joint[0].CurrentVel = motor_state[3].speed[1];

      joint[1].CurrentPos = motor_state[4].theta[0]; // shoulder
      joint[1].CurrentVel = motor_state[4].speed[0];
      joint[2].CurrentPos = motor_state[4].theta[1]; // -shoulder
      joint[2].CurrentVel = motor_state[4].speed[1];

      // X6 X4

      joint[3].CurrentPos = motor_state[5].theta[0];
      joint[3].CurrentVel = motor_state[5].speed[0];

      joint[4].CurrentPos = motor_state[5].theta[1];
      joint[4].CurrentVel = motor_state[5].speed[1];

      joint[5].CurrentPos = motor_state[6].theta[1]; // gripper
      joint[5].CurrentVel = motor_state[6].speed[1];
      ...
      if (runtime == 0.0)
      {
        ...
        // joint[0].InitVel = motor_state[3].speed[0];
        // // joint[0].RefTargetPos = joint[0].InitPos; // motor is not in use.

        joint[0].RefTargetPos = th_elbow; // can10 motor 2
        joint[0].InitPos = joint[0].CurrentPos;
        joint[0].InitVel = joint[0].CurrentVel;

        joint[1].RefTargetPos = -th_shoulder;   // can11 motor 1 2
        joint[1].InitPos = joint[1].CurrentPos; // 4 0
        joint[1].InitVel = joint[1].CurrentVel;
        joint[2].RefTargetPos = th_shoulder;
        joint[2].InitPos = joint[2].CurrentPos; // 4 1
        joint[2].InitVel = joint[2].CurrentVel;

        joint[3].RefTargetPos = 90 * D2R; // for yaw to pitch of joint[3]
        joint[3].InitPos = motor_state[5].theta[0];
        joint[3].InitVel = motor_state[5].speed[0];

        joint[4].RefTargetPos = -15 * D2R; // pitch
        joint[4].InitPos = motor_state[5].theta[1];
        joint[4].InitVel = motor_state[5].speed[1];

        joint[5].RefTargetPos = -300 * D2R; // opening
        joint[5].InitPos = motor_state[6].theta[1];
        joint[5].InitVel = motor_state[6].speed[1];
      }

      if (runtime < 3000.0)
      {
        joint[0].RefPos = func_1_cos(runtime, joint[0].InitPos, joint[0].RefTargetPos, 3000.0);
        joint[0].RefVel = dt_func_1_cos(runtime, joint[0].InitPos, joint[0].RefTargetPos, 3000.0);
      ...
      else if (runtime == 3000.0)
      {
        ...
        joint[5].RefTargetPos = 300 * D2R; // gripping motion
        joint[5].InitPos = motor_state[6].theta[1];
        joint[5].InitVel = motor_state[6].speed[1];
      }
      else if (3000.0 < runtime && runtime < 6000.0)
      {
        joint[5].RefPos = func_1_cos(runtime - 3000, joint[5].InitPos, joint[5].RefTargetPos, 3000.0);
        joint[5].RefVel = dt_func_1_cos(runtime - 3000, joint[5].InitPos, joint[5].RefTargetPos, 3000.0);
      }
      else if (runtime == 6000.0)
      {
        joint[0].RefTargetPos = -th_elbow;
        joint[0].InitPos = joint[0].CurrentPos;
        joint[0].InitVel = joint[0].CurrentVel;

        joint[1].RefTargetPos = th_shoulder;
        ...
        joint[3].RefTargetPos = -90 * D2R;
        joint[3].InitPos = motor_state[5].theta[0];
        joint[3].InitVel = motor_state[5].speed[0];

        joint[4].RefTargetPos = 15 * D2R;
        joint[4].InitPos = motor_state[5].theta[1];
        joint[4].InitVel = motor_state[5].speed[1];
      ...
      if (runtime > 9000.0)
      {
        ...
        std::lock_guard<std::mutex> lock(_mtx);
        cmd_vel_flag = true; // Disable cmd_vel callback after 9000ms
      }
    ...
    ComputeTorque();
    for (int i = 3; i < NUM_OF_CANABLE; i++)
    {
      for (int j = 0; j < NUM_OF_MOTOR; j++)
      {
...
void MainActiveNode::ComputeTorque()
{
  ...
  tau[3][1] = (float)((200. * (joint[0].RefPos - joint[0].CurrentPos) + 2 * (joint[0].RefVel - joint[0].CurrentVel)) * TauMap);

  tau[4][0] = (float)((200 * (joint[1].RefPos - joint[1].CurrentPos) + 2 * (joint[1].RefVel - joint[1].CurrentVel)) * TauMap);
  tau[4][1] = (float)((200 * (joint[2].RefPos - joint[2].CurrentPos) + 2 * (joint[2].RefVel - joint[2].CurrentVel)) * TauMap);

  // tau[4][0] = (float)((200. * (joint[2].RefPos - joint[2].CurrentPos) + 2 * (joint[2].RefVel - joint[2].CurrentVel)) * TauMap);

  tau[5][0] = (float)((200 * (joint[3].RefPos - joint[3].CurrentPos) + 2 * (joint[3].RefVel - joint[3].CurrentVel)) * TauMap); //
  tau[5][1] = (float)((200 * (joint[4].RefPos - joint[4].CurrentPos) + 2 * (joint[4].RefVel - joint[4].CurrentVel)) * TauMap);
  // tau[6][0] = (float)((200 * (joint[6].RefPos - joint[6].CurrentPos) + 2 * (joint[6].RefVel - joint[6].CurrentVel)) * TauMap); //
  tau[6][1] = (float)((200. * (joint[5].RefPos - joint[5].CurrentPos) + 2 * (joint[5].RefVel - joint[5].CurrentVel)) * TauMap);
  // tau[0][1] [][] [][] [][] [][] [][] [][] 합격.
