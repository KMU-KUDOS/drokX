...
MainActiveNode::MainActiveNode() : Node("Main_Active")
{
  ...
  for (int i = 0; i < NUM_OF_CANABLE; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      ...
      std::string temp_topic = "/can" + std::to_string(i + 10) + "/motor" + std::to_string(j + 1) + "_temp";
      std::string tau_topic = "/can" + std::to_string(i + 10) + "/motor" + std::to_string(j + 1) + "_tau";
      std::string speed_topic = "/can" + std::to_string(i + 10) + "/motor" + std::to_string(j + 1) + "_speed";
      std::string torque_topic = "/can" + std::to_string(i + 10) + "/motor" + std::to_string(j + 1) + "_torque";
      std::string motor_degree_topic = "/can" + std::to_string(i + 10) + "/motor" + std::to_string(j + 1) + "_degree";
...
void *MainActiveNode::pthread_arm()
{
  ...
  while ((!arm_thread_exit))
  {
    ...
    // //*example = use 1-cos func, run motor current pos to Ref target pos while 3.0 sec  and then fixed joint pos(angle)
    joint[0].CurrentPos = motor_state[0].theta[0]; // can10 motor id 1
    joint[0].CurrentVel = motor_state[0].speed[0];

    joint[1].CurrentPos = motor_state[1].theta[0]; // can11 motor id 1
    joint[1].CurrentVel = motor_state[1].speed[0];
    joint[2].CurrentPos = motor_state[1].theta[1]; // can11 motor id 2
    joint[2].CurrentVel = motor_state[1].speed[1];

    joint[3].CurrentPos = motor_state[0].theta[1]; // can10 motor id 2
    joint[3].CurrentVel = motor_state[0].speed[1];

    joint[4].CurrentPos = motor_state[2].theta[1]; // can12 motor id 1
    ...
    else
    {
      for (int i = 0; i < 6; i++)
      {
        joint[i].RefPos = joint[i].CurrentPos;
        joint[i].RefVel = joint[i].CurrentVel;
      }
    }
