...
void MainActiveNode::cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  double left_speed = (linear_velocity - (wheel_base / 2.0) * angular_velocity) / wheel_radius;
  double right_speed = (linear_velocity + (wheel_base / 2.0) * angular_velocity) / wheel_radius;
...
void *MainActiveNode::pthread_main()
{
  ...
  while ((!main_thread_exit))
  {
    ...
    for (int i = 1; i < 3; i++) // CAN IDs 5, 6 correspond to index 1~2
    {
...
void *MainActiveNode::pthread_arm()
{
  ...
  while ((!arm_thread_exit))
  {
    ...
    q0(5) = motor_state[7].theta[0]; // joint 구조체와 별개로, 행렬 연산을 위한 벡터
    ...
    if (runtime == 0.0)
    {
      ...
      joint[5].InitPos = motor_state[7].theta[0];
      joint[5].InitVel = motor_state[7].speed[0];
    }
    ...
    ComputeTorque();
    for (int i = 4; i < NUM_OF_CANABLE; i++) /// NUM_OF_CANABLE = 4   8
    {
      for (int j = 0; j < NUM_OF_MOTOR; j++)
      {
...
{
  std::lock_guard<std::mutex> lock(_mtx);  ///
  for (int i = 4; i < NUM_OF_CANABLE; i++) // i = 4; i = 0;
  {
...
{
  for (int i = 4; i < NUM_OF_CANABLE; i++) //   for (int i = 4; i < NUM_OF_CANABLE; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
