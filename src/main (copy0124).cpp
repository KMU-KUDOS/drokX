...
{
  struct period_info pinfo;
  pinfo.period_ns = ns_FiveMilSec; // ns_TwoMilSec
  usleep(500);
  clock_gettime(CLOCK_MONOTONIC, &(pinfo.next_period));
  ...
  while (!arm_thread_exit)
  {
    ...
    // Apply torque commands to manipulator
    // For CAN 10
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[4], j + 1, tau[4][j], frame[4]);
    }
    // For CAN 11
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[5], j + 1, tau[5][j], frame[5]);
    }
    // For CAN 12
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[6], j + 1, tau[6][j], frame[6]);
    }
    // For CAN 13
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[7], j + 1, tau[7][j], frame[7]);
    }
...
void MainActiveNode::ComputeTorque_arm()
{
  ...
  tau[6][0] = (Kp_w1 * (joint[8].RefPos - joint[8].CurrentPos) + Kd_w1 * (joint[8].RefVel - joint[8].CurrentVel)) * (TauMap / 3); // TauMap_X4 needed
  ...
  if (tau[6][0] > 25.) // 62.5 // 5
    tau[6][0] = 25.;
  else if (tau[6][0] < -25.)
    tau[6][0] = -25.;

  if (tau[6][1] > 62.5) // 62.5
    tau[6][1] = 62.5;
  else if (tau[6][1] < -62.5)
    tau[6][1] = -62.5;
