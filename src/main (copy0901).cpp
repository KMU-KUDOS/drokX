...
MainActiveNode::MainActiveNode() : Node("Main_Active")
{
  ...
  command_mode = POSITION_COMMAND; // TEST TORQUE_COMMAND POSITION_COMMAND RPM_COMMAND
  ...
  for (int i = 0; i < NUM_OF_CANABLE; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      flag_f[i][j] = 1;
      flag_b[i][j] = 1;
      previous_degree_f[i][j] = 0;
      previous_degree_b[i][j] = 0;
      angle_switch_f[i][j] = 0;
      angle_switch_b[i][j] = 0;
  ...
  for (int i = 0; i < NUM_OF_CANABLE; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      ...
      READ_TORQUE_SPEED_ANGLE(cansock[i], j + 1, frame[i]);
    }
  }
  std::cout << "==============================" << std::endl;
...
// }
int flipper_count[4] = {500, 500, 500, 500};
void MainActiveNode::joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    if (msg->buttons[i + 7] != 0)
    {
      if (flipper_upper == true)
      {
        if (i == 0 || i == 3)
          torque_command_[i] -= 2; // torque_command_ 배열 값 증가
        else
          torque_command_[i] += 2; // torque_command_ 배열 값 감소
      }
      else if (flipper_upper == false)
      {
        if (i == 0 || i == 3)
          torque_command_[i] += 2; // torque_command_ 배열 값 증가
        else
          torque_command_[i] -= 2; // torque_command_ 배열 값 크게 감소
      }
    }
    for (int i = 0; i < 4; i++)
    {
      if (torque_command_[i] > 100)
      {
        if (i % 2 != 0)
          torque_command_[i] = 300;
        else
          torque_command_[i] = 300;
      }
      else if (torque_command_[i] < -100)
      {
        if (i % 2 != 0)
          torque_command_[i] = -100;
        else
          torque_command_[i] = -100;
      }
    }
    // std::cout << flipper_upper<<"torque_command_[" << i << "]: " << torque_command_[i] << std::endl;
  }
...
void MainActiveNode::cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  linear_velocity = msg->linear.x * 300;
  angular_velocity = msg->angular.z * 300;

  double left_speed = ((linear_velocity - (wheel_base / 2.0) * angular_velocity) / (wheel_radius * 2.0 * M_PI)) * 250.0;
  double right_speed = ((linear_velocity + (wheel_base / 2.0) * angular_velocity) / (wheel_radius * 2.0 * M_PI)) * 250.0;
...
void *MainActiveNode::pthread_main()
{
  ...
  while ((!main_thread_exit))
  {
    ...
    joint[1].CurrentPos = motor_state[1].theta[1];
    joint[1].CurrentVel = motor_state[1].speed[1];

    joint[2].CurrentPos = motor_state[2].theta[0];
    joint[2].CurrentVel = motor_state[2].speed[0];
    ...
    if (runtime == 0.)
    {
      std::cout << "RUNTIME0000" << std::endl;
      joint[0].InitPos = motor_state[0].theta[0];
      joint[0].InitVel = motor_state[0].speed[0];

      joint[1].InitPos = motor_state[0].theta[1];
      joint[1].InitVel = motor_state[0].speed[1];

      joint[2].InitPos = motor_state[3].theta[0];
      joint[2].InitVel = motor_state[3].speed[0];

      joint[3].InitPos = motor_state[3].theta[1];
      joint[3].InitVel = motor_state[3].speed[1];
    }
    else
    {
      joint[0].RefPos = joint[0].RefPos / 2;
      joint[1].RefPos = joint[1].RefPos / 2;
      joint[2].RefPos = joint[2].RefPos / 2;
      joint[3].RefPos = joint[3].RefPos / 2;
    }
    ComputeTorque();
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[canable], motor, torque_command_[i], frame[i]);
    }
    ...
    if (time_since_last_joystick > 300)
    {
      ...
      for (int i = 0; i < 4; i++)
      {
        int canable = (i < 2) ? 0 : 3; // Front flippers use can4, Back flippers use can7
        int motor = (i % 2) + 1;       // Left flippers use motor 1, Right flippers use motor 2
        Rmd.TORQUE_CLOSED_LOOP_CONTROL(cansock[canable], motor, 0, frame[canable]);
      }
    }
...
void MainActiveNode::ComputeTorque()
{
  for (int i = 0; i < NUM_OF_CANABLE; i++) //   for (int i = 4; i < NUM_OF_CANABLE; i++)
  {
    for (int j = 0; j < NUM_OF_MOTOR; j++)
    {
      tau[i][j] = (int)((Kp_FL * (joint[i].RefPos - joint[i].CurrentPos) + Kd_FL * (joint[i].RefVel - joint[i].CurrentVel)) * TauMap);
      // (float)
      if (tau[i][j] > 62.5)
      {
        tau[i][j] = 62.5;
...
void MainActiveNode::READ_TORQUE_SPEED_ANGLE(int s, int canable, struct can_frame frame)
{
  ...
  if (FD_ISSET(s, &read_fds))
  {
    ...
    else if (nbytes == sizeof(struct can_frame))
    {
      ...
      if (flag_f[canable][motor_id - 1] == 1)
      {
        previous_degree_f[canable][motor_id - 1] = degree;
        flag_f[canable][motor_id - 1] = 0;
      }
      if (flag_b[canable][motor_id - 1] == 1)
      {
        previous_degree_b[canable][motor_id - 1] = degree;
        flag_b[canable][motor_id - 1] = 0;
      }

      if (degree - previous_degree_f[canable][motor_id - 1] < -20 && s == 16)
        angle_switch_f[canable][motor_id - 1] += 1;
      else if (degree - previous_degree_f[canable][motor_id - 1] > 20 && s == 16)
        angle_switch_f[canable][motor_id - 1] -= 1;

      if (degree - previous_degree_b[canable][motor_id - 1] < -20 && s == 19)
        angle_switch_b[canable][motor_id - 1] += 1;
      else if (degree - previous_degree_b[canable][motor_id - 1] > 20 && s == 19)
        angle_switch_b[canable][motor_id - 1] -= 1;

      // Update output degrees for both front and back
      if (s == 16)
        output_degree_f = angle_switch_f[canable][motor_id - 1] * 40. + degree;
      if (s == 19)
        output_degree_b = angle_switch_b[canable][motor_id - 1] * 40. + degree;

      // Check for bounds and reset if necessary
      if ((output_degree_f > 360 || output_degree_f < -360) && s == 16)
      {
        angle_switch_f[canable][motor_id - 1] = 0;
        output_degree_f = degree; // Reset to the current degree
      }

      if ((output_degree_b > 360 || output_degree_b < -360) && s == 19)
      {
        angle_switch_b[canable][motor_id - 1] = 0;
        output_degree_b = degree; // Reset to the current degree
      }
      if (s == 16)
        previous_degree_f[canable][motor_id - 1] = degree;
      if (s == 19)
      ...
      if (s == 16 || s == 19)
      {
        printf("CANABLE: %d CAN ID: 0x%x, Motor ID: %d\n", s, frame.can_id, motor_id);
        printf("Temp: %d, Torque: %d, Speed: %d, Degree: %f\n", temp, torque, speed, degree);
      }
      // Update motor_state based on CAN ID
      if (frame.can_id == 0x141)
      {
        if (canable == 0)
        {
          ...
          motor_state[canable].theta[0] = -(output_degree_f - 71.8) * D2R;
        }
        else if (canable == 1)
        {
          motor_state[canable].torque[0] = torque;
          motor_state[canable].speed[0] = speed * D2R / 9;
          motor_state[canable].theta[0] = output_degree_f * D2R;
        }
        else if (canable == 2)
        {
          motor_state[canable].torque[0] = -torque;
          motor_state[canable].speed[0] = -speed * D2R / 9;
          motor_state[canable].theta[0] = -(output_degree_b - 71.8) * D2R;
        }
        else if (canable == 3)
        {
          motor_state[canable].torque[0] = -torque;
          motor_state[canable].speed[0] = -speed * D2R / 9;
          motor_state[canable].theta[0] = -output_degree_b * D2R;
        }
      }
      else if (frame.can_id == 0x142)
      {
        if (canable == 0)
        {
          ...
          motor_state[canable].theta[1] = (output_degree_f + 71.78) * D2R;
        }
        else if (canable == 1)
        {
          motor_state[canable].torque[1] = -torque;
          motor_state[canable].speed[1] = -speed * D2R / 9;
          motor_state[canable].theta[1] = -(output_degree_b - 72.4) * D2R;
        }
        else if (canable == 2)
        {
          motor_state[canable].torque[1] = -torque;
          motor_state[canable].speed[1] = speed * D2R / 9;
          motor_state[canable].theta[1] = (output_degree_b + 72.32) * D2R;
        }
        else if (canable == 3)
        {
          motor_state[canable].torque[1] = -torque;
          motor_state[canable].speed[1] = -speed * D2R / 9;
          motor_state[canable].theta[1] = -(output_degree_b - 71.8) * D2R;
        }
      }
      else if (frame.can_id == 0x143)
      {
        if (canable == 0)
        {
          ...
          motor_state[canable].theta[2] = output_degree_f * D2R;
        }
        else if (canable == 1)
        {
          motor_state[canable].torque[2] = -torque;
          motor_state[canable].speed[2] = -speed * D2R / 9;
          motor_state[canable].theta[2] = -(output_degree_b + 160.59) * D2R;
        }
        else if (canable == 2)
        {
          motor_state[canable].torque[2] = -torque;
      ...
      // Store the final degree for future use
      // previous_degree_f[canable][motor_id - 1] = degree;
      // previous_degree_b[canable][motor_id - 1] = degree;
    }
  }
