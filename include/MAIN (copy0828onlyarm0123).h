...
#define NUM_OF_CANABLE 4 /// 8 팔만 할 때는 4 차체까지 하면 8
#define NUM_OF_MOTOR 2
#define JOINT_NUM 8 // 16 8 6
...
// void RMD_RPM(int32_t vesc_id, int32_t rpm);
...
private:
    ...
    {
        float theta_org[16]; // 8 : number of motors total. open RMD_COMMAND.h
        float theta[16];     // it might be 16. let's try it
        float speed[16];
        float torque[16];
        float temp[16];
        int data_count[4] = {0, 0, 0, 0}; // maybe 4 : theta, speed, torque, temp. why 6?
    };

    /*
    struct Motor_state_t
    {
        float theta[NUM_OF_MOTOR * NUM_OF_CANABLE];
        float speed[NUM_OF_MOTOR * NUM_OF_CANABLE];
        float torque[NUM_OF_MOTOR * NUM_OF_CANABLE];
        float temp[NUM_OF_MOTOR * NUM_OF_CANABLE];
        int data_count[NUM_OF_MOTOR * NUM_OF_CANABLE];
    };
    */
    ...
    rclcpp::Subscription<std_msgs::msg::Float32MultiArray>::SharedPtr torque_sub;
    int32_t torque_command_[4];

    int8_t temp = 0; ///
    int16_t torque = 0;
    int16_t speed = 0;
    int16_t pulse = 0;
    float pulse_to_degree = 0.00001525878 * 40.; /// 1:9 (360) 1:36 1:6
    float degree = 0.;
    ...
    float tau[NUM_OF_CANABLE][NUM_OF_MOTOR]; // int tau[NUM_OF_CANABLE][NUM_OF_MOTOR];
    ...
    // void get_flipper_info(int i, int &canable, int &motor); //
    void Check_State();
    void ComputeTorque();
    ...
    Motor_state_t Get_motor_state(int i) // num of canable : 4
    {
